...
import {
    PermissionsOverwriter__factory
} from "../typechain";
...
describe('Custom PermissionSet Overwriter tests', () => {
    ...
    it('should check the correct roles in their originals', async () => {
        ...
        expect(await permissionsOverwriter["hasRole(uint256,address)"](await permissions.TOKEN_ROLE_MINTER(), signers.ben.address)).to.be.false;
    })

    it('should fail with custom RoleID when it was not setup correctly', async () => {
        const { permissionsOverwriter, permissions, signers } = await loadFixture(deployPermissionsAwareMock);

        const transformedRoleId = await permissionsOverwriter.transformedRoleId(myCustomPermissionSetId, await permissions.TOKEN_ROLE_MINTER());
    ...
    it('should check the correct roles with overwritten roleIds', async () => {
        ...
        expect(await permissionsOverwriter["hasRole(uint256,address)"](await permissions.TOKEN_ROLE_MINTER(), signers.ben.address)).to.be.true;
    })
