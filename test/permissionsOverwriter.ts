...
describe("Custom PermissionSet", () => {
    ...
    it("should emit PermissionSetIdChanged-Event upon changing PermissionSetID", async () => {
        ...
        await expect(permissionsOverwriter.setPermissionSetId(myNewCustomPermissionSetId))
            ...
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_MINTER,
                affectedRoleIds.TOKEN_ROLE_MINTER.add(myDefaultCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenRemoved")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_WHITELIST_ADMIN,
                affectedRoleIds.TOKEN_ROLE_WHITELIST_ADMIN.add(myDefaultCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenRemoved")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_TRANSFERER,
                affectedRoleIds.TOKEN_ROLE_TRANSFERER.add(myDefaultCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenRemoved")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_OPERATOR,
                affectedRoleIds.TOKEN_ROLE_OPERATOR.add(myDefaultCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenRemoved")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_IS_WHITELISTED,
                affectedRoleIds.TOKEN_ROLE_IS_WHITELISTED.add(myDefaultCustomPermissionSetId * 1000)
            )

            .to.emit(permissionsOverwriter, "CustomRoleTokenAdded")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_MINTER,
                affectedRoleIds.TOKEN_ROLE_MINTER.add(myNewCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenAdded")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_WHITELIST_ADMIN,
                affectedRoleIds.TOKEN_ROLE_WHITELIST_ADMIN.add(myNewCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenAdded")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_TRANSFERER,
                affectedRoleIds.TOKEN_ROLE_TRANSFERER.add(myNewCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenAdded")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_OPERATOR,
                affectedRoleIds.TOKEN_ROLE_OPERATOR.add(myNewCustomPermissionSetId * 1000)
            )
            .to.emit(permissionsOverwriter, "CustomRoleTokenAdded")
            .withArgs(
                affectedRoleIds.TOKEN_ROLE_IS_WHITELISTED,
                affectedRoleIds.TOKEN_ROLE_IS_WHITELISTED.add(myNewCustomPermissionSetId * 1000)
            );
        ...
        expect(await permissionsOverwriter.getCustomRoleTokenAt(affectedRoleIds.TOKEN_ROLE_MINTER, 0)).to.eq(
            affectedRoleIds.TOKEN_ROLE_MINTER.add(myNewCustomPermissionSetId * 1000)
        );
        expect(await permissionsOverwriter.getCustomRoleTokens(affectedRoleIds.TOKEN_ROLE_MINTER)).to.be.eql([
            affectedRoleIds.TOKEN_ROLE_MINTER.add(myNewCustomPermissionSetId * 1000),
        ]);
        ...
        for (const tokenRoleIdIdx in affectedRoleIds) {
            ...
            expect(await permissionsOverwriter.getCustomRoleTokenAt(roleId, 0)).to.eq(
                roleId.add(myNewCustomPermissionSetId * 1000)
            );
        }
    });
});
